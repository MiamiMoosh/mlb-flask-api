...
def get_shop_data():
    ...
    try:
        ...
        for p in products:
            # Slugify the Printify title just to look it up in static metadata
            title_slug = slugify(p.get("title", ""))
            meta = static_metadata.get(title_slug)

            if meta:
                # Use your manually defined metadata
                p.update(meta)
                p["slug"] = meta["slug"]  # Force override of any autogenerated slug
            else:
                # Fall back to automated tagging + slugify
